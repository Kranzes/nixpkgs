...
- `pkgs.mkRepartImage` was added. It builds a disk image with `systemd-repart` from a module system configuration. The NixOS `image.repart` module uses it. See [](#sec-pkgs-mkRepartImage).
