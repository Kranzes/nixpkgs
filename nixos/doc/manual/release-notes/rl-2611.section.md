...
- `image.repart` is now a submodule of `pkgs.mkRepartImage`, which declares its options.
  `config.image.repart.image` no longer has an `override` function. Use `extendModules` to change the configuration and `overrideAttrs` to change the derivation.
  The `image.repart.imageFileBasename` and `image.repart.imageFile` aliases, renamed in 24.11, have been removed. Use `image.baseName` and `image.fileName`.
